...
typedef uint32_t bitset_base_t;

typedef struct bitset {
    bitset_base_t values; // множество
    bitset_base_t maxValue; // максимальный элемент универсума
} bitset;

// возвращает пустое множество c универсумом 0, 1,..., maxValue
bitset bitset_create(bitset_base_t maxValue);

// возвращает 'истина', если значение x может
// находиться в множестве set, иначе - 'ложь'
